...
import { Bell, BellRing, MessageSquare, Smartphone, AlertTriangle, Plus, Search, Trash2, Edit, RefreshCw, Download } from 'lucide-react';
...
export default function AdminNotifications() {
  ...
  const handleResendOneSignal = async (notificationId: string) => {
    ...
    if (!notification?.category) {
      toast.error('Notificação sem categoria não pode ser enviada via OneSignal');
      return;
    }
    ...
    try {
      ...
      toast.success('Notificação reenviada via OneSignal!');
      fetchNotifications();
    } catch (error) {
      console.error('Erro:', error);
      toast.error('Erro ao reenviar via OneSignal');
    }
  };
  ...
  const handleExportCSV = () => {
    const csvData = notifications.map(n => ({
      ...
      'OneSignal': n.onesignal_sent ? 'Enviado' : 'Não enviado'
    }));
    ...
    <AppLayout>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">OneSignal</CardTitle>
                      ...
                      <TableRow key={notification.id}>
                        ...
                        <TableCell>
                          <NotificationStatusBadge 
                            whatsappSent={notification.whatsapp_sent || false}
                            onesignalSent={notification.onesignal_sent || false}
                            hasError={!!notification.onesignal_error}
                          />
                        </TableCell>
                        <TableCell>
